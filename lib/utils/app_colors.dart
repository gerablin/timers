import 'dart:ui';

import 'package:flutter/material.dart';

class AppColors {
  static const Color primaryColor = Color(0xff1C3879);
  static const Color secondaryColor = Color(0xff7796C2);
  static const Color accentColor = Color(0xffEAE3D2);
  static const Color white = Color(0xffffffff);
  static const Color alternativeAccentColor = Color(0xffF9F5EB);
  static const Color darkBackground = Color(0xff0E1117);
  static const Color lightBackgroundColor = Color(0xff2F3136);
  static const Color disabledColor = Colors.green;
  static const Color textColor = Color(0xffBBBBBF);
}
